...
<div class="row">
  <div class="col-lg-3 mb-4">
    <a href="{{route('income.create')}}" class="btn btn-primary" id="tambahPemasukan">Tambah Pemasukan</a>
  </div>
</div>
  ...
  <table class="table table-hover col-lg-12 table-bordered table-striped dataTables" id="table-data">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{$val->date}}</td>
        <td>{{$val->asset->asset}}</td>
        <td>{{$val->account->account}}</td>
        <td>{{$val->total}}</td>
        <td>{{$val->info}}</td>
        <td>
          <form action="{{route('income.destroy', $val->id)}}" method="POST">
            {{-- @if (Auth::check()) --}}
            <a class="btn btn-success btn-sm" href="{{ route('income.edit', $val->id) }}">
